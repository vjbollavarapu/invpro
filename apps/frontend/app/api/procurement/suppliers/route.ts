...
// GET /api/procurement/suppliers - Fetch all suppliers
export async function GET(request: NextRequest) {
  try {
    ...
    const backendResponse = await fetch(
      `${API_URL}/procurement/suppliers/?${params.toString()}`,
      {
    ...
    if (!backendResponse.ok) {
      const error = await backendResponse.json().catch(() => ({ error: "Failed to fetch suppliers" }))
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!backendResponse.ok) {
      const error = await backendResponse.json().catch(() => ({ error: "Failed to create supplier" }))
      return NextResponse.json({ success: false, error: error.error || error.detail }, { status: backendResponse.status })
    }
