...
// GET /api/finance/cost-centers - Fetch cost centers
export async function GET(request: NextRequest) {
  try {
    const response = await fetch(
      `${API_URL}/finance/cost-centers/`,
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        { error: error.error || error.detail || "Failed to fetch cost centers" },
...
export async function POST(request: NextRequest) {
  try {
    ...
    const response = await fetch(`${API_URL}/finance/cost-centers/`, {
      method: 'POST',
      headers: getAuthHeaders(request),
    ...
    if (!response.ok) {
      const error = await response.json().catch(() => ({}))
      return NextResponse.json(
        { error: error.error || error.detail || "Failed to create cost center" },
        { status: response.status }
      )
    }
